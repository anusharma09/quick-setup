    ...
    public class ExcelReport : IDisposable
    {
        ...
        private void CreateExcelFile(ExcelFile reportName)
        {
            ...
            try
            {
                ...
                {
                    Exception ex = new Exception("Job does not have actual Hours or Quantity");
                    throw ex;
                }
                if (oXl == null)
                    oXl = new Microsoft.Office.Interop.Excel.Application();
        ...
        // AND SUBSTRING(JobChangeOrderDescription,1,2) <> 'X~' 
            else
            ...
            query = "SELECT JobChangeOrderID, JobChangeOrderNumber, JobChangeOrderStatus, JobChangeOrderDescription, " +
                    ...
                    " WHERE JobID = " + jobID + "  " + // AND SUBSTRING(JobChangeOrderDescription,1,2) <> 'X~'  " +
            ...
            try
            {
                ...
                if (perid.Trim().Length > 0)
                     query = " SELECT DISTINCT JobCostCodePhaseID, JobCostCodePhase As [Phase], CostCode AS [Code] , " +
                        ...
                        " FROM tblJobCostCodePhaseHistory p  WHERE JobID = '" + jobID + "' AND Period = '" + perid + "' Order BY JobCostCodePhase + CostCode DESC ";
