...
namespace JCCBusinessLayer
{
    public class Job
    {
        ...
        private string completedOps;
        private string glAutoWC;
        private string profLiab;
        private string completedOpsYears;
        private string glAutoWCYears;
        private string profLiabYears;
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(estimateNumber) && String.IsNullOrEmpty(jobNumber))
                {
                    if (!FileSystem.DirectoryExists(@sourceEstimateLocation))
                    {
                        ...
                        {
                            CopyFiles(CCEApplication.FormsLocation, sourceEstimateLocation, false, true);
                        }
                    }
                }
                // -----------------------------------------------------
                    ...
                    if (!FileSystem.DirectoryExists(@sourceJobLocation))
                    {
                        ...
                        else
                        {
                            if (FileSystem.DirectoryExists(@sourceEstimateLocation))
                            {
                                ...
                                if (!FileSystem.DirectoryExists(@sourceJobLocation + "Budget\\"))
                                {
                                    Directory.CreateDirectory(sourceJobLocation + "Budget\\");
                                    Directory.CreateDirectory(sourceJobLocation + "DailyReports\\");
                                    Directory.CreateDirectory(sourceJobLocation + "Safety\\");
                                    Directory.CreateDirectory(sourceJobLocation + "ScheduleOfValue\\");
                                    Directory.CreateDirectory(sourceJobLocation + "TimeCards\\");
                                }
        ...
        private void CopyFiles(string source, string destination, bool origFolder, bool isEstimate)
        {
            try
            {
                if (isEstimate)
                {
                    ...
                    Directory.CreateDirectory(destination + "Addendums\\");
                    Directory.CreateDirectory(destination + "Contracts\\");
                    Directory.CreateDirectory(destination + "Drawings\\");
                    Directory.CreateDirectory(destination + "Pictures\\");
                    Directory.CreateDirectory(destination + "Quotes\\");
                    Directory.CreateDirectory(destination + "RFIs\\");
                    Directory.CreateDirectory(destination + "Specs\\");
                    Directory.CreateDirectory(destination + "Submittals\\");
                    Directory.CreateDirectory(destination + "TakeOffSheets\\");
                    Directory.CreateDirectory(destination + "Templates\\");
                ...
                else
                {
                    if (!Directory.Exists(destination))
                    {
                        ...
                        String tempFolder = destination.Substring(0, destination.Length - 15) + "\\";

                        Directory.CreateDirectory(tempFolder + "Budget\\");
                        Directory.CreateDirectory(tempFolder + "DailyReports\\");
                        Directory.CreateDirectory(tempFolder + "Safety\\");
                        Directory.CreateDirectory(tempFolder + "ScheduleOfValue\\");
                        Directory.CreateDirectory(tempFolder + "TimeCards\\");
