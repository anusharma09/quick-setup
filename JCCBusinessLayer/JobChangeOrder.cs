...
namespace JCCBusinessLayer
{
    public class JobChangeOrder
    {
        ...
        {
            if (StarbuilderSynching.IsSynchingActiveWithStarbuilder)
            {
            ...
            else
            {
                try
                {
                    var query = " INSERT INTO tblJobChangeOrderSyncWithStarBuilder"
                                  + "(JobNumber, ChangeOrderNumber, Date, SpName,Source)"
                                   + "VALUES "
                                   + "(" + jobID.ToString() + ",0,'" + DateTime.Now + "', 'up_DMJobUpdateStarbuilderPrimaryContract','SG3')";
            ...
            else
            {
                try
                {
                    var query = " INSERT INTO tblJobChangeOrderSyncWithStarBuilder"
                                  + "(JobNumber, ChangeOrderNumber, Date, SpName,Source)"
                                   + "VALUES "
                                   + "(" + jobID.ToString() + "," + jobChangeOrderID + ",'" + DateTime.Now + "', 'up_DMJobUpdateStarbuilderChangeOrder','SG3')";
        ...
        public static bool UpdateChangeOrderDefaultLabelValues(string jobID, string BIMRate_Label, string apprenticeLaborRate_Label, string electricianLaborRate_Label, string foremanLaborRate_Label, string generalForemanLaborRate_Label, string superintendentLaborRate_Label, string projectManagerLaborRate_Label, string projectEngineerLaborRate_Label, string safetyMeetingsLaborRate_Label, string premiumTimeLaborRate_Label,string  SafetyMeetingPercent_Label,string FringeBenefitsPercent_Label)
        {
            ...
            else
            {
                try
                {
                    var query = " INSERT INTO tblJobChangeOrderSyncWithStarBuilder"
                                  + "(JobNumber, ChangeOrderNumber, Date, SpName,Source)"
                                   + "VALUES "
                                   + "(" + jobID.ToString() + ",0,'" + DateTime.Now + "', 'up_DMJobUpdateStarbuilderPrimaryContractCostCodes','SG3')";
            ...
            else
            {
                try
                {
                    var query = " INSERT INTO tblJobChangeOrderSyncWithStarBuilder"
                                  + "(JobNumber, ChangeOrderNumber, Date, SpName,Source)"
                                   + "VALUES "
                                   + "(" + jobID.ToString() + "," + jobChangeOrderNumber + ",'" + DateTime.Now + "', 'up_DMJobUpdateStarbuilderChangeOrderCostCodes','SG3')";
        ...
        //
        //
        public static DataSet GetJobChangeOrderDetailRev(string jobChangeOrderID, string revision)
        {
            string query = "";
            if (jobChangeOrderID == "")
                jobChangeOrderID = "0";
            ...
            query = "SELECT " +
                    ...
                    " LEFT JOIN tblJobChangeOrderRev C " +
                    " ON j.JobID = c.JobID " +
                    " LEFT JOIN tblCustomer u " +
                    " ON j.CustomerID = u.CustomerID " +
                    " LEFT JOIN tblJobCostCodeRev d " +
                    " ON c.JobChangeOrderID = d.JobChangeOrderID AND c.Rev = d.Rev " +
                    " LEFT JOIN tblJobCostCodePhaseRev p " +
                    " ON d.JobCostCodePhaseID = p.JobCostCodePhaseID and d.Rev = p.Rev " +
                    " WHERE c.JobChangeOrderID = '" + jobChangeOrderID + "' AND c.Rev = '" + revision + " '";
            try
            {
                return DataBaseUtil.ExecuteDataset(query, CCEApplication.Connection, CommandType.Text);
            }
